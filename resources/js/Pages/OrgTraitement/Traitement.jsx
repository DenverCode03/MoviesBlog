...
export default function Traitement() {
    ...
    const handleFilterChange = (filterType, value) => {
        ...
        router.get(route('orgtraitement.traitement'), newFilters, {
            preserveState: true,
            preserveScroll: true
        });
    };
    ...
    const handleSearch = (e) => {
        e.preventDefault();
        router.get(route('orgtraitement.traitement'), { ...filters, search: searchTerm }, {
            preserveState: true,
            preserveScroll: true
        });
    };
    ...
    const handlePrendreEnCharge = (requete) => {
    ...
    return (
        <Base title="Traitement des Requêtes" user={auth.user}>
            <Head title="Traitement des Requêtes" />
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                    {[
                        { key: 'en_traitement', label: 'À traiter', color: 'blue', value: stats.en_traitement },
                        { key: 'traitees', label: 'Traitées', color: 'purple', value: stats.traitees },
                        { key: 'terminees', label: 'Terminées', color: 'green', value: stats.terminees },
                        { key: 'rejetees', label: 'Rejetées', color: 'red', value: stats.rejetees },
                        { key: 'urgentes', label: 'Urgentes', color: 'orange', value: stats.urgentes }
                    ].map((stat) => (
                        <div 
                            key={stat.key}
                            onClick={() => handleFilterChange('statut', stat.key === 'en_traitement' ? 'en_traitement_organisme' : stat.key === 'traitees' ? 'traitee_organisme' : stat.key === 'rejetees' ? 'rejetee_organisme' : stat.key)}
                            className={`cursor-pointer rounded-xl p-4 border-2 transition-all hover:shadow-md ${
                                (filters.statut === 'en_traitement_organisme' && stat.key === 'en_traitement') ||
                                (filters.statut === 'traitee_organisme' && stat.key === 'traitees') ||
                                (filters.statut === 'rejetee_organisme' && stat.key === 'rejetees') ||
                ...
                <div className="bg-white border border-gray-200 rounded-xl p-6">
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between space-y-4 lg:space-y-0">
                        <div>
                            <h2 className="text-2xl font-bold text-gray-900">Requêtes à traiter - {getOrganismeName(auth.user.role)}</h2>
                            <p className="text-gray-600">Traitez les requêtes approuvées par la direction</p>
                        </div>
                            ...
                            <div className="flex flex-col lg:flex-row lg:items-center justify-between space-y-4 lg:space-y-0">
                                ...
                                <div className="flex items-center space-x-3">
                                    ...
                                    {requete.statut === 'en_traitement_organisme' && (
                                        <button
                                            onClick={() => handlePrendreEnCharge(requete)}
                                    ...
                                    {(requete.statut === 'en_cours_organisme' || requete.statut === 'traitee_organisme') && (
                                        <Link
                                            href={route('orgtraitement.requetes.show', requete.id)}
                ...
                {requetes.data.length === 0 && (
                    <div className="text-center py-12">
                        ...
                        <p className="text-gray-500">
                            {searchTerm || filters.statut !== 'en_traitement_organisme' || filters.priorite !== 'all' || filters.type_requete !== 'all'
                                ? 'Essayez de modifier vos critères de recherche.'
                                : 'Il n\'y a actuellement aucune requête à traiter.'
                            }
                        </p>
                    </div>
                )}
